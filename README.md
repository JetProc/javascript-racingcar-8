...
2. `Validator`를 통해 입력받은 값들의 **유효성 검사**

3. 유효성 검사를 통과하면, 이 입력값들을 `Race` 클래스로 전달하여 새로운 **`Race` 인스턴스 생성**

4. `OutputView`를 통해 결과 `실행 결과` **헤더 문구 출력**

5. 입력받은 `시도 횟수`만큼 **`for` 루프 실행**

   1. `Race` 모델의 **`playRound()` 메서드 호출**
   ...
   2. `OutputView`를 통해 **현재 라운드의 실행 결과 출력**

6. `for` 루프가 종료되면, `Race` 모델의 `getWinners()` 메서드를 호출하여 **최종 우승자 이름 배열 받기**

7. `OutputView`를 통해 **최종 우승자 출력**
...
1. **자동차 이름 입력 (`validateCarNames`)**
   ...
   - **이름 포맷**
     - [ ] 이름에 숫자나 특수문자가 포함된 경우 (`pobi,woni,jun!`, `car1,car2`)
     - [ ] 이름이 숫자로만 이루어진 경우 (`123,456`)
